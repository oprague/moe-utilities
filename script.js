...
function onEtherianChange() {
	...
	const startingLevel = etherianInfo[etherianName]['startingLevel'];
...
function calculate() {
	...
	if (!(document.getElementById('all-etherians').checked)){
		const startingLevel = etherianInfo[etherianName]['startingLevel'];
		const expClass = etherianInfo[etherianName]['expClass'];
		expTicks = prestige(expGain, startingLevel, expClass, playerStartingLevel, startingPrestige, targetPrestige);
	} else {
		for (const [key, value] of Object.entries(etherianInfo)) {
			const startingLevel = etherianInfo[key]['startingLevel'];
			const expClass = etherianInfo[key]['expClass'];
